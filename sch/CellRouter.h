...
struct Pin {
	...
	// minimum distance from the [left, right] side of the cell
	array<int, 2> offset;
	// maximum distance (without moving anything else) from the [left, right]
	// side of the cell
	array<int, 2> bound;
...
struct Contact {
	...
	// Minimum required spacing from other pins to/from this via. "pin" indexes
	// into Subckt::stack, "offset" is distance in dbunits.
	// <from, offset>
	// |==|==|
	//       O
	//  ---->
...
struct CellRouter {
	...
	array<bool, 2> unresolvedPinCycle;
	...
	vector<StackConstraint> stackConstraints;
	...
	void propagateOrderMap(vector<bitset> &m, set<int> todo=set<int>());
	vector<bitset> routeOrderMap(int type);
	vector<bitset> pinOrderMap();

	vector<set<int> > createAdjacencyList();
	...
	bool buildRouteOffsets(int type, vector<int> start=vector<int>());
	void alignPins();
	bool assignStackConstraints();
